package com.securitygateway.loginboilerplate.service.implementation;
...
import com.securitygateway.loginboilerplate.payload.responses.GeneralAPIResponse;
import com.securitygateway.loginboilerplate.payload.responses.RegisterResponse;
import com.securitygateway.loginboilerplate.payload.responses.RegisterVerifyResponse;
import com.securitygateway.loginboilerplate.payload.responses.UserProfile;
import com.securitygateway.loginboilerplate.repository.UserRepository;
import com.securitygateway.loginboilerplate.service.AuthenticationService;
import com.securitygateway.loginboilerplate.service.EmailService;
import com.securitygateway.loginboilerplate.service.JwtService;
import com.securitygateway.loginboilerplate.service.OtpService;
    ...
    public ResponseEntity<?> loginUser(LoginRequest loginRequest) {
        ...
        try {
            ...
            if (!user.getIsVerified()) {
                String otpToBeMailed = otpService.getOtpForEmail(email);
                CompletableFuture<Integer> emailResponse = emailService.sendEmailWithRetry(loginRequest.getEmail().trim().toLowerCase() ,otpToBeMailed);
                if (emailResponse.get() == -1) {
                    return new ResponseEntity<>(RegisterResponse.builder()
                            .message("Failed to send OTP email. Please try again later.")
                            .build(), HttpStatus.INTERNAL_SERVER_ERROR);
